#!/bin/bash
# pcap + live sk_e dump of the IKE_FOLLOWUP_KE fragments (no source instrumentation).
# Isolated ring; tcpdump the responder veth; after ESTABLISHED, gdb-dump the
# responder's sk_e_i (AES-256-CBC key) so the fragments can be decrypted offline.
set -u
D=/tmp/r2key; CAP="$D/cap.pcap"; KEY="$D/sk_e_i.hex"
PREFIX=/usr/local/racoon2-i2i; SBIN=$PREFIX/sbin; CONF=/tmp/r2i2i_boot
...
ip netns exec "$NSR" tcpdump -i "$VR" -nn -s 0 udp port 500 -w "$CAP" >"$D/td.log" 2>&1 &
TCPID=$!
sleep 2
"$SBIN/ikedctl" -s /tmp/iked.sock-i2i establish-sa isakmp inet "$HI" "$HR" sel_out >/dev/null 2>&1 || true
# after IKE_SA is up (key material set), read sk_e_i from the live responder
sleep 6
RKPID=$(pgrep -f "sbin/iked .*responder.conf" | head -1)
echo "=== gdb dump sk_e_i of responder pid $RKPID ==="
gdb -q -batch -p "$RKPID" \
    -ex 'set $s = ikev2_sa_list.tqh_first' \
    -ex 'dump binary memory '$D'/sk_e_i.bin $s->sk_e_i->v ($s->sk_e_i->v + $s->sk_e_i->l)' \
    -ex 'dump binary memory '$D'/sk_e_r.bin $s->sk_e_r->v ($s->sk_e_r->v + $s->sk_e_r->l)' \
    -ex 'printf "sk_e_i len=%u\n", (unsigned int)$s->sk_e_i->l' \
    -ex 'printf "sk_e_r len=%u\n", (unsigned int)$s->sk_e_r->l' 2>&1 | grep -E "sk_e_." | tee "$KEY"
ls -l "$D"/sk_e_i.bin "$D"/sk_e_r.bin 2>/dev/null
xxd -p "$D"/sk_e_i.bin 2>/dev/null | tr -d '\n' >> "$KEY"
printf " " >> "$KEY"
xxd -p "$D"/sk_e_r.bin 2>/dev/null | tr -d '\n' >> "$KEY"
...
echo "CAP=$CAP size=$(wc -c < "$CAP" 2>/dev/null)"
echo "DONE"
exit 0
